...
void Rigid_Body::Collide_Cube_Ground(float l)
{
    ...
    if(max_depth<0){
        ...
        vec3 delta_vp = -vp_n * (1 + coefficient_of_restitution);
        vec3 vp_rel = delta_vp - vp_t; //TODONE: incorporate tangential component of velocity
        vec3 impulse= K_inverse * vp_rel; //NOTE: this impulse is calculated without friction
        
        // calculate j_stick
        mat33 negative_k_inverse = -K_inverse;
        vec3 j_stick = negative_k_inverse * vp_rel;
        
//        std::cout <<"j_stick (" << j_stick[0] << ", " << j_stick[1] << ", " << j_stick[2] << ")" << std::endl;

        // determine if j_stick is in friction cone
        bool in_friction_cone = (j_stick - ((j_stick * normal) * normal)).length() <= (coef_of_friction * j_stick) * normal;

        static bool entered_once = false;
        
        // if it isn't, compute the correct impulse with sliding friction
        if (!in_friction_cone){
            vec3 j_n = (delta_vp * normal) / (normal * (K * (normal - coef_of_friction * tangent)));
            vec3 j = j_n * normal - coef_of_friction * j_n * tangent;
            impulse = j;
        } else {
            entered_once = true;
        }
